export const MAZE_EXAMPLE = `

EXAMPLE MAZE (no real physics - used custom collisionSystem)
...
export var mazeGame = function() {
  let maze = new MatrixEngineWGPU({
    ...
    function generateMazeLogic(meshes) {
      ...
      for(let y = 0;y < mazeSize;y++) {
        for(let x = 0;x < mazeSize;x++) {
          if(grid[y][x] === 0) {
            ...
            maze.collisionSystem.registerStatic((test.name), test.position, 1, 'walls');
          }
        }
      }
      ...
      maze.collisionSystem.registerCamera(app.cameras.firstPersonCamera.position, 0.5);
