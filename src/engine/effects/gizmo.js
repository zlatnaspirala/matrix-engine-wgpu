...
_handleDrag(mouseEvent) {
  ...
  // ✅ Improved movement scale based on screen sensitivity
  const movementScale = 0.3; // Increased from 0.05
  
  // ✅ Combined movement for better control
  const totalMovement = Math.sqrt(deltaX * deltaX + deltaY * deltaY);
  const direction = deltaX > Math.abs(deltaY) ? deltaX : -deltaY;
  
  switch (this.dragAxis) {
    case 1: // X axis - horizontal movement
      this.parentMesh.position.x += deltaX * movementScale;
      break;
      
    case 2: // Y axis - vertical movement (inverted because screen Y is inverted)
      this.parentMesh.position.y -= deltaY * movementScale;
      break;
      
    case 3: // Z axis - combined movement
      // Use the dominant direction for Z
...
_rayIntersectsLine(rayOrigin, rayDir, lineStart, lineEnd, threshold) {
  ...
  const a = ray[0]**2 + ray[1]**2 + ray[2]**2; // Should be 1 (normalized)
  ...
  // Check if closest point on line segment (tc should be between 0 and 1)
  ...
  const dist = Math.sqrt(
    (closestOnRay[0] - closestOnLine[0])**2 +
    (closestOnRay[1] - closestOnLine[1])**2 +
    (closestOnRay[2] - closestOnLine[2])**2
  );
