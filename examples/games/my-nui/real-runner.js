...
+      // Removed automatic wall generator (was creating static walls that blocked runners)
...
@@
       app.lightContainer[0].setPosition(0, 35, 0);
       menuBeast.lightContainer[0].setTarget(0, 0, -20);

+      // Runner system: obstacles that approach the player from +Z and loop back to +Z
+      // when they pass beyond a minimum Z. X is continuous in a range (not discrete paths).
...
+          minY: 1,
+          maxY: 5,
+          startZ: 40,   // spawn in front (+Z)
+          endZ: -60,    // when reaching this Z (past player) send back to startZ
+          speedMin: 0.4,
+          speedMax: 1.2,
+          scaleMin: 0.7,
+          scaleMax: 2.2
...
+          const z = cfg.startZ + Math.random() * 20; // spread in Z so they don't all align
...
+                r.position.z = r._runnerCfg.startZ + Math.random() * 20;
...
+                const s = rand(r._runnerCfg.scaleMin, r._runnerCfg.scaleMax);
+                if (r.scale) r.scale = [s, s, s];
+              }
+            }
+          }
+        };
