...
export var loadHandBeast = function() {
  ...
  let loadHand = new MatrixEngineWGPU({
    ...
    const nui = new PipeCommander(true, null, null, {enableVisual: false});
    ...
    async function onLoadObj(m) {
      ...
      setTimeout(async () => {
        ...
        MYCUBE.effects.splat = new GaussianSplatScene(loadHand.device, 'rgba16float', loadHand.cameraBuffer);
        ...
        loadHand.autoUpdate.push(animator);
        ...
        loadHand.autoUpdate.push(positionAnimator);
        positionAnimator.setMode('hold');


        const handEffect = new SplatHandEffect(
          loadHand.device,
          'rgba16float',
          loadHand.cameraBuffer,
          MYCUBE.effects.splat.splatLayers[0],
          {scale: 3.0, clusterRadius: 0.08, origin: [0, 1.2, 0]}
        );

        // Attach to mesh effects — main loop calls updateInstanceData + render automatically
        MYCUBE.effects.handSplat = handEffect;

        // // Hook mediapipe into it
        nui.onResults = (results) => {
            console.log('hands detected:', results?.landmarks?.length ?? 0);
          MYCUBE.effects.handSplat.setHandData(results);
        };
