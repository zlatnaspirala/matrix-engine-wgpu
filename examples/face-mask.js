...
export var loadFaceMask = function() {
  ...
  let FACEMASK = new MatrixEngineWGPU({
    ...
    async function onLoadObj(m) {
      ...
      setTimeout(() => {
        // alert()
        FACEMASK.MYCUBE.effects.fancy =
          new SacredGeometryEffect(FACEMASK.device, 'rgba16float'  ,FACEMASK.cameraBuffer);

        FACEMASK.lightContainer[0].setIntensity(14);
        // FACEMASK.lightContainer[1].setIntensity(14);

        FACEMASK.activateBloomEffect();
        FACEMASK.bloomPass.setBlurRadius(16);
        // FACEMASK.activateVolumetricEffect({
        //   density: 0.03,
        //   steps: 32,
        //   scatterStrength: 0.8,
        ...
        // });
        // FACEMASK.activateHZB();
        FACEMASK.lightContainer[0].setPosition(0, 45, -10);
        FACEMASK.lightContainer[0].setTarget(0, 0, -10);
        // FACEMASK.lightContainer[1].setPosition(0, 5, -10);
        // FACEMASK.lightContainer[1].setTarget(0, 5, 10);
        ...
        // FACEMASK.MYCUBE.effects.facemask.init()
    ...
    FACEMASK.canvas.addEventListener("ray.hit.event", (e) => {
