// Matter.js Physics Worker
// Mirrors the bridge protocol from jolt-physics-worker.js
...
class MatterPhysicsWorker {
  ...
  async init(options = {}) {
    ...
    // Import Matter.js from CDN or local
    // For worker: use dynamic import or bundled version
    const Matter = await import('https://cdnjs.cloudflare.com/ajax/libs/matter-js/2.0.19/matter.min.js').catch(() => {
      // Fallback: assume Matter is globally available
      return self.Matter;
    });
    ...
    this.Events.on(this.engine, 'collisionStart', (event) => {
      for (const pair of event.pairs) {
        ...
        self.postMessage({
          ...
          normal: [normal.x, normal.y, 0] // 2D → [x, y, 0]
        });
      }
    });
  ...
  step() {
    ...
    for (let i = 0; i < this.rigidBodies.length; i++) {
      ...
      snap[base + 7] = body.isStatic ? 0 : 1; // active flag
    }
  }
